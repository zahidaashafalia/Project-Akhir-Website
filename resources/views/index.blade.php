...
<!-- VOUCHERS -->
<section class="section-sm" style="background:#fff;padding:48px 0">
    <div class="container">
        <div class="section-header">
            <div>
                <h2 class="section-title">🎟️ Voucher & Promo</h2>
                <p class="section-subtitle">Hemat lebih banyak dengan voucher eksklusif</p>
            </div>
            <a href="{{ route('promo') }}" class="section-link">Lihat semua <i class="fas fa-chevron-right"></i></a>
        </div>
        ...
        <div class="grid-2">
            @foreach($activeVouchers as $voucher)
            <div class="voucher-item" onclick="copyVoucher('{{ $voucher->code }}', this)">
                <div class="voucher-icon-wrap">
                    {{ $voucher->type === 'free_delivery' ? '🛵' : ($voucher->type === 'percentage' ? '💯' : '💰') }}
                </div>
                <div class="voucher-body">
                    ...
                    <div class="voucher-info">
                        Min. order Rp {{ number_format($voucher->min_order, 0, ',', '.') }}
                        @if($voucher->end_date) • s.d {{ $voucher->end_date->format('d M Y') }} @endif
                    </div>
                </div>
                <button class="voucher-code" onclick="event.stopPropagation();copyVoucher('{{ $voucher->code }}', this)">
                    {{ $voucher->code }}
                </button>
            </div>
            @endforeach
            ...
            @foreach([
                ['💯','MAKANKUFIRST','Diskon 30%','Untuk Pengguna Baru','Min. order Rp 30.000','percentage'],
                ['🛵','ONGKIRGRATIS','Gratis Ongkir','Semua Restoran','Min. order Rp 20.000','delivery'],
                ['💰','HEMAT15','Hemat 15%','Semua Menu','Min. order Rp 50.000','percentage'],
                ['🎂','ULTAHKU','Diskon 50%','Di hari ulang tahunmu','Min. order Rp 25.000','special'],
            ] as $v)
            <div class="voucher-item" onclick="copyVoucher('{{ $v[1] }}', this)">
                <div class="voucher-icon-wrap">{{ $v[0] }}</div>
                <div class="voucher-body">
                    <div class="voucher-discount">{{ $v[2] }}</div>
                    <div class="voucher-name">{{ $v[3] }}</div>
                    <div class="voucher-info">{{ $v[4] }}</div>
                </div>
                <button class="voucher-code">{{ $v[1] }}</button>
            </div>
            @endforeach
            @endif
        </div>
...
<section class="section">
    <div class="container">
        <div class="section-header">
            <div>
                <h2 class="section-title">🆕 Restoran Baru</h2>
                <p class="section-subtitle">Temukan pengalaman kuliner yang baru dan segar</p>
            </div>
        </div>
        <div class="scroll-row">
            @foreach($newRestaurants as $restaurant)
            <a href="{{ route('restaurant.show', $restaurant->slug) }}" class="restaurant-card" style="width:220px">
                <div class="restaurant-card-img-wrap" style="height:150px">
                    <img src="{{ $restaurant->banner_url }}" alt="{{ $restaurant->name }}" loading="lazy">
                    <span class="restaurant-card-badge badge-new">🆕 Baru</span>
                </div>
                <div class="restaurant-card-body">
                    <div class="restaurant-name">{{ $restaurant->name }}</div>
...
<section class="section" style="background:#fff;padding:64px 0">
    <div class="container">
        <div style="text-align:center;margin-bottom:48px">
            <h2 class="section-title" style="font-size:32px">Cara Pesan di MakanKu</h2>
            <p style="color:var(--text-gray);font-size:15px;margin-top:8px">Mudah, cepat, dan anti ribet!</p>
        </div>
        <div class="how-steps">
            <div class="how-step">
                ...
                <h4>Pilih Lokasi</h4>
                <p>Aktifkan GPS atau masukkan alamat pengirimanmu</p>
            </div>
            <div class="how-step">
                <span class="how-step-num">2</span>
                <span class="how-step-icon">🍽️</span>
                <h4>Pilih Makanan</h4>
                <p>Browse restoran dan pilih menu favoritmu</p>
            </div>
            <div class="how-step">
                <span class="how-step-num">3</span>
                <span class="how-step-icon">💳</span>
                <h4>Bayar & Konfirmasi</h4>
                <p>Bayar dengan berbagai metode pembayaran yang tersedia</p>
            </div>
            <div class="how-step">
                <span class="how-step-num">4</span>
                <span class="how-step-icon">🛵</span>
                <h4>Terima Pesanan</h4>
                <p>Lacak pesananmu secara real-time hingga sampai di tanganmu!</p>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="container">
        <div class="cta-banner">
            <div class="cta-content">
                <h2>Punya Usaha Kuliner? Bergabunglah!</h2>
                <p>Daftarkan restoran atau warung kamu dan jangkau ribuan pelanggan baru setiap harinya.</p>
            </div>
            <div class="cta-actions">
                <a href="/partner/register" class="btn btn-primary btn-lg"><i class="fas fa-store"></i> Daftar Restoran</a>
                <a href="/partner/driver" class="btn" style="background:rgba(255,255,255,0.15);color:#fff;border:2px solid rgba(255,255,255,0.3)" onmouseover="this.style.background='rgba(255,255,255,0.25)'" onmouseout="this.style.background='rgba(255,255,255,0.15)'"><i class="fas fa-motorcycle"></i> Jadi Mitra Kurir</a>
            </div>
        </div>
    </div>
</section>
...
<section class="section-sm" style="padding:48px 0">
    <div class="container">
        <div style="text-align:center;margin-bottom:36px">
            ...
            <p style="color:var(--text-gray);margin-top:6px">Sudah dipercaya oleh lebih dari 100.000 pelanggan</p>
        </div>
        <div class="grid-3">
            @foreach([
                ['⭐⭐⭐⭐⭐','Aplikasi pesan makanan terbaik! Pengirimannya cepat banget, makanannya selalu datang masih panas. Pilihan restorannya juga banyak banget!','Budi S.','Pelanggan sejak 2023','👨'],
                ['⭐⭐⭐⭐⭐','Voucher dan promonya banyak banget, sering banget dapet gratis ongkir. Customer servicenya juga responsif kalau ada masalah.','Siti R.','Super Customer','👩'],
                ['⭐⭐⭐⭐⭐','Fitur live trackingnya keren banget, bisa lihat kurir di mana. Tampilan appnya juga modern dan enak dipakai. Highly recommended!','Agus P.','Food Enthusiast','👨‍💼'],
            ] as $t)
            <div class="testimonial-card">
                ...
                <div class="testimonial-author">
                    ...
                    <div>
                        ...
                        <div class="testimonial-meta">{{ $t[3] }}</div>
                    </div>
                </div>
            </div>
            @endforeach
...
document.getElementById('heroSearch').addEventListener('keypress', e => { if (e.key === 'Enter') doHeroSearch(); });
...
// Auto scroll promo slider
const slider = document.getElementById('promoSlider');
if (slider) {
    let scrollPos = 0;
    setInterval(() => {
        scrollPos += slider.querySelector('.promo-slide')?.offsetWidth + 20 || 440;
